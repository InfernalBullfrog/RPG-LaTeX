...
% Start document
\begin{document}
\fontfamily{T1LibreBskvl-LF}\selectfont % Set text font

\begin{titlepage}
    \rpgtitlepage{{\color{red} RPG}\\CATCHY ADVENTURE NAME} % Title
                 {black} % Background color
                 {img/generic-cover-image.jpg} % Path to cover art
                 {\color{white} COVER TEXT HERE} % Subtitle or other cover text
                 {\color{white} \large Written by InfernalBullfrog} % Author
\end{titlepage}
\newpage \clearpage
\thispagestyle{empty}
\mbox{}
